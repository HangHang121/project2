...
def betterEvaluationFunction(currentGameState):
    ...
    # Calculate the score from the current game state
    ...
    if foodList:
        closestFoodDist = min([manhattanDistance(newPos, foodPos) for foodPos in foodList])
        score += 1.0 / closestFoodDist  # Encourage closer foods

    # Adjust the score based on ghost positions and scared times
    ghostDistances = [manhattanDistance(newPos, ghost.getPosition()) for ghost in newGhostStates]
    for dist, scaredTime in zip(ghostDistances, newScaredTimes):
        if scaredTime > 0:  # If the ghost is scared
            score += 10.0 / dist  # Encourage chasing scared ghosts
        else:
            if dist < 2:  # Avoid ghosts that are too close
                score -= 100

    # Consider the number of remaining foods as a penalty to encourage eating
